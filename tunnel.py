...
class Tunnel:
    ...
    def __init__(self, speed=1):
        ...
        for x in range(self.size):
            self.pillars.append(
                [self.current_pillar_height - 1, 136 - self.total_pillar_length + self.current_pillar_height + 1,
                 (x - 1) * 8])
    ...
    def update(self):
        ...
        if self.pillars[0][2] < -8:
            if not (
            self.pattern_length) or self.current_pillar_height < 1 or self.current_pillar_height > self.total_pillar_length - 2:
